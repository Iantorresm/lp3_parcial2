...
typedef struct {
    ...
    int generated_by_producer[PRODUCER_COUNT];
    int prepared_by_cook[COOK_COUNT];
    int delivered_by_delivery_person[DELIVERY_PERSON_COUNT];
    unsigned int total_preparation_seconds;
...
static int delivery_system_init(DeliverySystem *system)
{
    ...
    system->total_preparation_seconds = 0;

    for (index = 0; index < PRODUCER_COUNT; index++) {
        system->generated_by_producer[index] = 0;
    }
    for (index = 0; index < COOK_COUNT; index++) {
        system->prepared_by_cook[index] = 0;
    }
    for (index = 0; index < DELIVERY_PERSON_COUNT; index++) {
        system->delivered_by_delivery_person[index] = 0;
    }
...
static void register_generated_order(DeliverySystem *system, int producer_id)
{
    lock_mutex(&system->counters_mutex, "bloquear estadísticas");
    system->generated_orders++;
    system->generated_by_producer[producer_id - 1]++;
    unlock_mutex(&system->counters_mutex, "desbloquear estadísticas");
}

static void register_prepared_order(DeliverySystem *system, int cook_id, const Order *order)
{
    lock_mutex(&system->counters_mutex, "bloquear estadísticas");
    system->prepared_orders++;
    system->prepared_by_cook[cook_id - 1]++;
    system->total_preparation_seconds += order->preparation_seconds;
    unlock_mutex(&system->counters_mutex, "desbloquear estadísticas");
}

static void register_delivered_order(DeliverySystem *system, int delivery_id)
{
    lock_mutex(&system->counters_mutex, "bloquear estadísticas");
    system->delivered_orders++;
    system->delivered_by_delivery_person[delivery_id - 1]++;
    unlock_mutex(&system->counters_mutex, "desbloquear estadísticas");
}
...
static void *producer_thread(void *argument)
{
    ...
    for (index = 0; index < ORDERS_PER_PRODUCER; index++) {
        ...
        register_generated_order(context->system, context->worker_id);
...
static void *cook_thread(void *argument)
{
    ...
    while (1) {
        ...
        snprintf(
            message,
            sizeof(message),
            "[Cocinero %d] Preparando pedido %d",
            context->worker_id,
            order.id);
        print_event(context->system, message);
        ...
        register_prepared_order(context->system, context->worker_id, &order);
    }
}
...
static void *delivery_thread(void *argument)
{
    ...
    while (1) {
        ...
        snprintf(
            ...
            order.id);
        print_event(context->system, message);
        register_delivered_order(context->system, context->worker_id);
    }
}
...
{
    int index;
    double average_preparation_seconds = 0.0;

    lock_mutex(&system->counters_mutex, "bloquear estadísticas");
    if (system->prepared_orders > 0) {
        average_preparation_seconds =
            (double)system->total_preparation_seconds / (double)system->prepared_orders;
    }
    ...
    printf("Preparados: %d\n", system->prepared_orders);
    printf("Entregados: %d\n", system->delivered_orders);
    printf("Promedio de preparación: %.2f segundos\n", average_preparation_seconds);

    printf("\nPor productor:\n");
    for (index = 0; index < PRODUCER_COUNT; index++) {
        printf("- Productor %d: %d pedidos\n", index + 1, system->generated_by_producer[index]);
    }

    printf("\nPor cocinero:\n");
    for (index = 0; index < COOK_COUNT; index++) {
        printf("- Cocinero %d: %d pedidos\n", index + 1, system->prepared_by_cook[index]);
    }

    printf("\nPor repartidor:\n");
    for (index = 0; index < DELIVERY_PERSON_COUNT; index++) {
        printf(
            "- Repartidor %d: %d pedidos\n",
            index + 1,
            system->delivered_by_delivery_person[index]);
    }
...
int main(void)
{
    ...
    print_event(&system, "Simulacro 3: estadísticas por thread.");
