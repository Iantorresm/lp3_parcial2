...
#define MAX_ORDERS_PER_PRODUCER 10
#define RESTAURANT_OPEN_SECONDS 4
...
typedef struct {
    ...
    pthread_mutex_t restaurant_state_mutex;
...
static int delivery_system_init(DeliverySystem *system)
{
    ...
    result = pthread_mutex_init(&system->restaurant_state_mutex, NULL);
    if (result != 0) {
    ...
    if (result != 0) {
        pthread_mutex_destroy(&system->restaurant_state_mutex);
        order_queue_destroy(&system->ready_orders);
        order_queue_destroy(&system->pending_orders);
        return result;
    }
...
static int restaurant_is_open(DeliverySystem *system)
{
    int is_open;
    lock_mutex(&system->restaurant_state_mutex, "bloquear estado del restaurante");
    is_open = system->restaurant_is_open;
    unlock_mutex(&system->restaurant_state_mutex, "desbloquear estado del restaurante");
    return is_open;
}

static void close_restaurant(DeliverySystem *system)
{
    lock_mutex(&system->restaurant_state_mutex, "bloquear estado del restaurante");
    system->restaurant_is_open = 0;
    unlock_mutex(&system->restaurant_state_mutex, "desbloquear estado del restaurante");
}
...
static void *producer_thread(void *argument)
{
    ...
    for (index = 0; index < MAX_ORDERS_PER_PRODUCER; index++) {
        char message[256];
        int id;
        Order order;

        if (!restaurant_is_open(context->system)) {
            snprintf(
                message,
                sizeof(message),
                "[Productor %d] Restaurante cerrado: deja de generar pedidos",
                context->worker_id);
            print_event(context->system, message);
            return NULL;
        }

        id = next_order_id(context->system);
        order = create_order(id, context->worker_id);

        snprintf(
            message,
            sizeof(message),
            "[Productor %d] Aceptó pedido %d (%s)",
            context->worker_id,
            order.id,
...
static void *cook_thread(void *argument)
{
    ...
    while (1) {
        ...
        snprintf(message, sizeof(message), "[Cocinero %d] Prepara pedido %d", context->worker_id, order.id);
        print_event(context->system, message);
...
static void *delivery_thread(void *argument)
{
    ...
    while (1) {
        ...
        snprintf(message, sizeof(message), "[Repartidor %d] Entrega pedido %d", context->worker_id, order.id);
        print_event(context->system, message);
...
static void print_summary(DeliverySystem *system)
{
    lock_mutex(&system->print_mutex, "bloquear impresión");
    printf("\nResumen simulacro 5\n");
    printf("El restaurante cerró después de %d segundos.\n", RESTAURANT_OPEN_SECONDS);
    printf("Pedidos aceptados antes del cierre: %d\n", system->generated_orders);
...
int main(void)
{
    ...
    print_event(&system, "Simulacro 5: cierre del restaurante con drenaje de pedidos aceptados.");
    ...
    sleep(RESTAURANT_OPEN_SECONDS);
    close_restaurant(&system);
    print_event(&system, "[Sistema] Restaurante cerrado: no se aceptan pedidos nuevos.");
