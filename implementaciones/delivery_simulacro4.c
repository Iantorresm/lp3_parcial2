...
typedef struct {
    ...
    sem_t completed_delivery_results;
...
static int delivery_system_init(DeliverySystem *system)
{
    ...
    if (sem_init(&system->completed_delivery_results, 0, 0) == -1) {
        result = errno;
    ...
    if (result != 0) {
        sem_destroy(&system->completed_delivery_results);
        order_queue_destroy(&system->ready_orders);
        order_queue_destroy(&system->pending_orders);
        return result;
    }
...
static int should_delivery_fail(const Order *order)
{
    /*
     * Regla determinística para practicar sin rand():
     * - IDs múltiplos de 5 fallan siempre y terminan cancelados.
     * - Otros pedidos pueden fallar de forma transitoria.
     */
    if (order->id % 5 == 0) {
        return 1;
    }

    return ((order->id + order->delivery_attempts) % 4) == 0;
}
...
static void register_delivered_order(DeliverySystem *system)
{
    ...
    post_sem(&system->completed_delivery_results, "avisar pedido finalizado");
}

static void register_canceled_order(DeliverySystem *system)
{
    lock_mutex(&system->counters_mutex, "bloquear contadores");
    system->canceled_orders++;
    unlock_mutex(&system->counters_mutex, "desbloquear contadores");
    post_sem(&system->completed_delivery_results, "avisar pedido finalizado");
}
...
static void wait_until_all_orders_are_delivered_or_canceled(DeliverySystem *system)
{
    int expected_terminal_orders = PRODUCER_COUNT * ORDERS_PER_PRODUCER;
    int index;

    for (index = 0; index < expected_terminal_orders; index++) {
        wait_sem(&system->completed_delivery_results, "esperar pedido entregado o cancelado");
    }
}
...
static void *delivery_thread(void *argument)
{
    ...
    while (1) {
        ...
        if (should_delivery_fail(&order)) {
            if (order.delivery_attempts >= MAX_DELIVERY_ATTEMPTS) {
                snprintf(
                    message,
                    sizeof(message),
                    "[Repartidor %d] Canceló pedido %d luego de %d intentos",
                    context->worker_id,
                    order.id,
                    order.delivery_attempts);
                print_event(context->system, message);
                register_canceled_order(context->system);
            } else {
                snprintf(
                    message,
                    sizeof(message),
                    "[Repartidor %d] Falló pedido %d, reintento %d/%d",
                    context->worker_id,
                    order.id,
                    order.delivery_attempts,
            ...
            snprintf(
                message,
                sizeof(message),
                "[Repartidor %d] Entregó pedido %d en intento %d",
                context->worker_id,
                order.id,
                order.delivery_attempts);
            print_event(context->system, message);
            register_delivered_order(context->system);
        }
    }
}
...
int main(void)
{
    ...
    print_event(&system, "Simulacro 4: entregas fallidas con reintentos y cancelación.");
    ...
    wait_until_all_orders_are_delivered_or_canceled(&system);
